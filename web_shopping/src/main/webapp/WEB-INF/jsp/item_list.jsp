...
<body>
	...
	<c:forEach var="item" items="${items}">
		<form action="<%=PropertyLoader.getProperty("url.servlet.ShoppingServlet")%>?action=add_cart_item" method="post">
			商品コード：${item.itemCode}<br>
			商品名：${item.itemName}<br>
			単価：${item.itemPrice} 円<br>
			個数：
