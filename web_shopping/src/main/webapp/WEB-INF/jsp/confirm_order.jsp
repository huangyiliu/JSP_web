...
<body>
	...
	<c:if test="${not empty cart.itemListForReference}">
		...
		<c:set var="toOA" value="${orderedItem}" scope="session" />
		<!-- お客様情報 -->
		<h3>【お客様情報】</h3>
		<form
			action="<%=PropertyLoader.getProperty("url.servlet.ShoppingServlet")%>?action=request_order"
			method="post">
			<table>
				<tr>
					<th>お名前</th>
